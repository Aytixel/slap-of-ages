...
extern map_t *createMap(window_t *window, int map_size)
{
    ...
    map->map_tree_sprite = loadSprite(window, "asset/sprite/map/Trees.png");

    if (map->map_tree_sprite == NULL)
    {
        free(map);
...
extern void renderMap(window_t *window, map_t *map)
{
    ...
    map->tile_size = window->height / (map->map_size + 12);

    int offset = map->tile_size * ((float)map->map_size / 2 - 1);

    // calcul le nombre de case pour remplir la moitié de la fenêtre
    int halft_width_tile_count = window->width / 2 / map->tile_size - map->map_size / 2 + 1;
    int halft_height_tile_count = window->height / 2 / map->tile_size - map->map_size / 2 + 1;

    // affichage du sol de la carte
    for (int y = -halft_height_tile_count; y < map->map_size + halft_height_tile_count; y++)
    {
        for (int x = -halft_width_tile_count; x < map->map_size + halft_width_tile_count; x++)
        {
    ...
    for (int y = -halft_height_tile_count; y < map->map_size + halft_height_tile_count + 8; y++)
    {
        for (int x = -halft_width_tile_count; x < map->map_size + halft_width_tile_count + 7; x++)
        {
            // texture de l'arbre moyen par default
            SDL_Rect tree_source_rect = map->map_sprite_rects.medium_tree;
            SDL_Rect tree_tile_rect = map->map_sprite_tile_rects.medium_tree;
            ...
            case -3:
            case 4:
                tree_source_rect = map->map_sprite_rects.large_tree;
                tree_tile_rect = map->map_sprite_tile_rects.large_tree;
                break;
            case -2:
            case 2:
            case 3:
                tree_source_rect = map->map_sprite_rects.small_tree;
                tree_tile_rect = map->map_sprite_tile_rects.small_tree;
                break;
            case -5:
            case -7:
                tree_source_rect = map->map_sprite_rects.small_dead_tree;
                tree_tile_rect = map->map_sprite_tile_rects.small_dead_tree;
                break;
            case -4:
            case -6:
            case 0:
                tree_source_rect = map->map_sprite_rects.medium_dead_tree;
                tree_tile_rect = map->map_sprite_tile_rects.medium_dead_tree;
                break;
            case 5:
            case 7:
                tree_source_rect = map->map_sprite_rects.large_dead_tree;
                tree_tile_rect = map->map_sprite_tile_rects.large_dead_tree;
                break;
            default:
                break;
            }
            ...
            if ((
                    x < -1 ||
                    x > map->map_size + tree_tile_rect.w - 1 ||
                    y < -1 ||
                    y > map->map_size + tree_tile_rect.h - 1) &&
                (x * y * 3 + x - y) % 9 == 0)
            {
...
extern int deleteMap(map_t **map)
{
    ...
    destroySprite(&(*map)->map_tree_sprite);
    free(*map);
    *map = NULL;
